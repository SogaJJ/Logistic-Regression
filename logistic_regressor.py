...
class LogisticRegressor:
    ...
    def loss(self, *args):
        ...
        J = ((-np.array([y])).dot(np.log(utils.sigmoid(X.dot((np.array([theta])).T))))[0,0]-(1-np.array([y])).dot(np.log(1-utils.sigmoid(X.dot((np.array([theta])).T))))[0,0])/m
        return J
    ...
    def grad_loss(self, *args):
        ...
        grad = ((utils.sigmoid((np.array([theta])).dot(X.T))-np.array([y])).dot(X))[0]/m
    ...
    def predict(self, X):
        ...
        ###########################################################################
        y_pred = utils.sigmoid((np.array([self.theta])).dot(X.T))[0]
...
class RegLogisticRegressor:
    ...
    def loss(self, *args):
        ...
        ##########################################################################
        J = ((-np.array([y])).dot(np.log(utils.sigmoid(X.dot((np.array([theta])).T))))[0,0]-(1-np.array([y])).dot(np.log(1-utils.sigmoid(X.dot((np.array([theta])).T))))[0,0])/m + reg*np.array([theta]).dot(np.array([theta]).T)[0,0]/(2*m)
