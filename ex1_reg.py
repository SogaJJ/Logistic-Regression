...
accuracy = 1- np.nonzero(np.round(predy - y))[0].size/float(XX.shape[0])
...
# compare the loss and the number of non-zero coefficients of L1 and L2 regularization for variable lamda
print "when lamda = ", reg, ". Loss with L2 regularization is ", reg_lr1.loss(sk_logreg_l2.coef_[0],XX,y,0.0)
print "when lamda = ", reg, ". Loss with L1 regularization is ", reg_lr1.loss(sk_logreg_l1.coef_[0],XX,y,0.0)
print "when lamda = ", reg, ". The number of non-zero coefficients in L2 regularization is ", (sk_logreg_l2.coef_[0])[sk_logreg_l2.coef_[0]!=0].size  
print "when lamda = ", reg, ". The number of non-zero coefficients in L1 regularization is ", (sk_logreg_l1.coef_[0])[sk_logreg_l1.coef_[0]!=0].size